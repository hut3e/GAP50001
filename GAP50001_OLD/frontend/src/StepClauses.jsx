...
import { C, GAP_CHECKLIST, SCORE_CFG, CAT_CFG, WEIGHT_CFG, SPACE, RADIUS, FONT } from "./gap.ui.constants.js";
import { Tag, ScorePicker, KPIBar, Modal, Btn } from "./gap.atoms.jsx";
...
const SUB_NAMES = {
  ...
  "7.1":"Nguồn lực","7.2":"Năng lực","7.3":"Nhận thức","7.4":"Trao đổi thông tin","7.5":"Tài liệu",
...
const EMPTY_CUSTOM = { id:"", clause:"", title:"", weight:2, cat:"doc", legal:"", _custom:true };

export default function StepClauses({ survey, setSurvey, surveyId, apiUrl, setToast, ensureSurveyId }) {
  ...
  // ── Custom clauses CRUD ──────────────────────────────────────────
  const customClauses = Array.isArray(survey.custom_clauses) ? survey.custom_clauses : [];
  const setCustomClauses = (arr) => setSurvey(p => ({ ...p, custom_clauses: arr }));

  const [customModal, setCustomModal] = useState(null);
  // { mode: "add"|"edit", targetSub: "4.1", item: { id, clause, title, weight, cat, legal, _custom } }

  const openAddCustom = (subClause) => {
    // Generate next sequential ID not already used in standard + custom list
    const allForSub = [...GAP_CHECKLIST, ...customClauses].filter(i => i.clause === subClause);
    const maxN = allForSub.reduce((m, i) => {
      const n = parseInt(i.id.split(".").pop()) || 0;
      return Math.max(m, n);
    }, 0);
    const newId = `${subClause}.${maxN + 1}`;
    setCustomModal({
      mode: "add",
      targetSub: subClause,
      item: { ...EMPTY_CUSTOM, id: newId, clause: subClause },
    });
  };

  const openEditCustom = (item) => {
    setCustomModal({ mode:"edit", targetSub: item.clause, item: { ...item } });
  };

  const saveCustomModal = () => {
    if (!customModal) return;
    const { item, mode } = customModal;
    if (!item.title.trim()) { alert("Vui lòng nhập tiêu đề điều khoản."); return; }
    if (mode === "add") {
      setCustomClauses([...customClauses, item]);
    } else {
      setCustomClauses(customClauses.map(c => c.id === item.id ? item : c));
    }
    setCustomModal(null);
  };

  const deleteCustom = (id) => {
    if (!confirm("Xóa tiêu chí tùy chỉnh này? Điểm đánh giá liên kết sẽ vẫn còn trong responses.")) return;
    setCustomClauses(customClauses.filter(c => c.id !== id));
  };

  const setCustomField = (field, val) =>
    setCustomModal(p => p ? { ...p, item: { ...p.item, [field]: val } } : p);
  ...
  const stats = useMemo(() => {
    const all = [...GAP_CHECKLIST, ...customClauses];
    ...
    return { total:all.length, scored:n, crit, maj, min, good, needImproveCount, needImprovePct, customCount: customClauses.length };
  }, [resp, customClauses]);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      ...
      <KPIBar items={[
        ...
        { label:"Tùy chỉnh",       value:stats.customCount, col:C.violet, icon:"✚" },
      ...
      {CLAUSES.map(cl => {
        const clNum = cl.num.replace("§","");
        const clItemsStd = GAP_CHECKLIST.filter(i => i.clause.startsWith(clNum));
        const clItemsCustom = customClauses.filter(i => i.clause.startsWith(clNum));
        const clItems = [...clItemsStd, ...clItemsCustom];
        ...
        return (
          <div key={cl.num} style={{ background:C.bg2, borderRadius:10, overflow:"hidden",
            ...
            <div onClick={() => setOpen(p=>({...p,[cl.num]:!isOpen}))}
              ...
              {clItemsCustom.length > 0 && (
                <span style={{ background:`${C.violet}18`, color:C.violet, border:`1px solid ${C.violet}35`,
                  borderRadius:4, padding:"1px 7px", fontSize:FONT.label, fontWeight:600 }}>
                  +{clItemsCustom.length} tùy chỉnh
                </span>
              )}
              {avg && (
            ...
            {isOpen && (
              <div style={{ padding:14 }}>
                {cl.subs.map(sub => {
                  const subItemsStd    = clItemsStd.filter(i => i.clause === sub);
                  const subItemsCustom = clItemsCustom.filter(i => i.clause === sub);
                  const subItems = [...subItemsStd, ...subItemsCustom];
                  ...
                  return (
                    <div key={sub} style={{ marginBottom:14 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6,
                        ...
                        {/* ✚ Thêm tiêu chí tùy chỉnh vào sub-clause này */}
                        <button type="button" onClick={e => { e.stopPropagation(); openAddCustom(sub); }}
                          title={`Thêm tiêu chí tùy chỉnh vào §${sub}`}
                          style={{ display:"inline-flex", alignItems:"center", gap:4, padding:"3px 9px",
                            background:`${C.violet}18`, border:`1px solid ${C.violet}40`,
                            color:C.violet, borderRadius:5, fontSize:FONT.label, fontWeight:700, cursor:"pointer" }}>
                          ✚ Thêm tiêu chí
                        </button>
                      </div>
                      ...
                      {subItems.map((item, idx) => {
                        ...
                        return (
                          <div key={item.id} style={{ marginBottom:8, padding:10,
                            background:sc ? `${cfg.col}08` : isCustom ? `${C.violet}06` : C.bg3,
                            border:`1px solid ${sc ? cfg.col : isCustom ? C.violet : C.bd2}28`,
                            borderRadius:7, position:"relative" }}>
                            {/* Custom badge */}
                            {isCustom && (
                              <div style={{ position:"absolute", top:6, right:6, display:"flex", gap:4 }}>
                                <span style={{ background:`${C.violet}22`, color:C.violet, border:`1px solid ${C.violet}40`,
                                  borderRadius:4, padding:"1px 6px", fontSize:FONT.caption, fontWeight:700 }}>
                                  ✚ Tùy chỉnh
                                </span>
                                <button type="button" onClick={() => openEditCustom(item)} title="Sửa tiêu chí tùy chỉnh"
                                  style={{ padding:"2px 7px", background:`${C.blue}22`, border:`1px solid ${C.blue}40`,
                                    color:C.blueL, borderRadius:4, fontSize:FONT.caption, fontWeight:700, cursor:"pointer" }}>✏️</button>
                                <button type="button" onClick={() => deleteCustom(item.id)} title="Xóa tiêu chí tùy chỉnh"
                                  style={{ padding:"2px 7px", background:`${C.red}22`, border:`1px solid ${C.red}40`,
                                    color:C.redL, borderRadius:4, fontSize:FONT.caption, fontWeight:700, cursor:"pointer" }}>✕</button>
                              </div>
                            )}
                            {/* Item header */}
                            <div style={{ display:"flex", alignItems:"flex-start", gap:8, marginBottom:6,
                              paddingRight: isCustom ? 140 : 0 }}>
                              <span style={{ fontFamily:"'Fira Code',monospace", fontSize:FONT.label,
                                color: isCustom ? C.violet : C.blueL, flexShrink:0, marginTop:2 }}>{item.id}</span>
                              <div style={{ flex:1 }}>
      ...
      {/* ══════════════════════════════════════════════════════
          Modal: Thêm / Sửa điều khoản đánh giá tùy chỉnh
          ══════════════════════════════════════════════════════ */}
      <Modal
        open={!!customModal}
        onClose={() => setCustomModal(null)}
        title={customModal?.mode === "add"
          ? `✚ Thêm tiêu chí tùy chỉnh vào §${customModal?.targetSub}`
          : `✏️ Sửa tiêu chí: ${customModal?.item?.id}`}
        width={640}
      >
        {customModal && (
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            {/* Help text */}
            <div style={{ padding:"10px 14px", background:`${C.violet}12`, borderRadius:8,
              border:`1px solid ${C.violet}30`, fontSize:FONT.body, color:C.t2, lineHeight:1.6 }}>
              Thêm tiêu chí đánh giá bổ sung cho <strong style={{ color:C.violet }}>§{customModal.targetSub}</strong>.
              Tiêu chí tùy chỉnh được lưu vào database, hiển thị cùng các tiêu chí chuẩn ISO 50001.
              ID được tự động tạo theo thứ tự nhưng có thể chỉnh sửa.
            </div>

            {/* ID + Sub-clause */}
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              <div>
                <div style={{ fontSize:FONT.label, color:C.t2, fontWeight:700, textTransform:"uppercase", marginBottom:6 }}>
                  Mã điều khoản (ID) <span style={{ color:C.red }}>*</span>
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <span style={{ fontFamily:"'Fira Code',monospace", fontSize:16, color:C.violet,
                    background:`${C.violet}15`, border:`1px solid ${C.violet}40`, borderRadius:6,
                    padding:"6px 12px", fontWeight:700 }}>
                    {customModal.item.id}
                  </span>
                  <span style={{ fontSize:FONT.label, color:C.t3 }}>(tự động tạo)</span>
                </div>
              </div>
              <div>
                <div style={{ fontSize:FONT.label, color:C.t2, fontWeight:700, textTransform:"uppercase", marginBottom:6 }}>
                  Thuộc điều khoản cha
                </div>
                <span style={{ fontFamily:"'Fira Code',monospace", fontSize:15, color:C.blueL,
                  background:`${C.blue}15`, border:`1px solid ${C.blue}30`, borderRadius:6,
                  padding:"8px 14px", fontWeight:600, display:"inline-block" }}>
                  §{customModal.item.clause} — {SUB_NAMES[customModal.item.clause] || customModal.item.clause}
                </span>
              </div>
            </div>
            ...
            <div>
              <div style={{ fontSize:FONT.label, color:C.t2, fontWeight:700, textTransform:"uppercase", marginBottom:6 }}>
                Tiêu đề / Yêu cầu đánh giá <span style={{ color:C.red }}>*</span>
              </div>
              <textarea
                value={customModal.item.title}
                onChange={e => setCustomField("title", e.target.value)}
                placeholder="Nhập mô tả yêu cầu đánh giá đầy đủ, rõ ràng…"
                rows={3}
                style={{ width:"100%", background:C.bg3, border:`1px solid ${C.bd0}`,
                  borderRadius:8, padding:"8px 12px", color:C.t0, fontSize:FONT.body,
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              <div>
                ...
                <div style={{ display:"flex", gap:6 }}>
                  {[[1,"Cơ bản",C.tealL],[2,"Cao",C.amberL],[3,"Quan trọng",C.redL]].map(([v,l,col]) => (
                    <button key={v} type="button" onClick={() => setCustomField("weight", v)}
                      style={{ flex:1, padding:"8px 4px", borderRadius:6, fontSize:FONT.label, fontWeight:700,
                        cursor:"pointer", border:`1px solid ${col}50`,
                        background: customModal.item.weight === v ? col : `${col}15`,
                        color: customModal.item.weight === v ? "#fff" : col }}>
                      {l}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <select value={customModal.item.cat}
                  onChange={e => setCustomField("cat", e.target.value)}
                  style={{ width:"100%", minHeight:44, padding:"8px 12px", background:C.bg3,
                    border:`1px solid ${C.bd0}`, borderRadius:8, color:C.t0, fontSize:FONT.body,
                    fontFamily:"inherit" }}>
                  ...
                  <option value="legal">⚖️ Pháp lý VN</option>
                </select>
              </div>
            </div>
            ...
            <div>
              <div style={{ fontSize:FONT.label, color:C.t2, fontWeight:700, textTransform:"uppercase", marginBottom:6 }}>
                Tham chiếu pháp lý (tùy chọn)
              </div>
              <input
                value={customModal.item.legal}
                onChange={e => setCustomField("legal", e.target.value)}
                placeholder="vd: TT 25/2020/TT-BCT §5, Nghị định 30/2026 Điều 12…"
                style={{ width:"100%", minHeight:44, padding:"8px 12px", background:C.bg3,
                  border:`1px solid ${C.bd0}`, borderRadius:8, color:C.t0, fontSize:FONT.body,
                  fontFamily:"inherit" }}
              />
            ...
            <div style={{ display:"flex", justifyContent:"flex-end", gap:8, paddingTop:8,
              borderTop:`1px solid ${C.bd0}` }}>
              <button type="button" onClick={() => setCustomModal(null)}
                style={{ padding:"8px 18px", background:"transparent", border:`1px solid ${C.bd0}`,
                  color:C.t1, borderRadius:8, fontSize:FONT.body, fontWeight:600, cursor:"pointer" }}>
                Hủy
              </button>
              <button type="button" onClick={saveCustomModal}
                style={{ padding:"8px 20px", background:`linear-gradient(135deg,${C.violet},#9f5ff5)`,
                  border:"none", color:"#fff", borderRadius:8, fontSize:FONT.body, fontWeight:700, cursor:"pointer" }}>
                {customModal.mode === "add" ? "✚ Thêm tiêu chí" : "💾 Lưu thay đổi"}
              </button>
            </div>
          </div>
        )}
