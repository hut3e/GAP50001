...
import { C, FONT, SPACE, RADIUS, GAP_CHECKLIST } from "./gap.ui.constants.js";
...
/**
 * Socket.IO URL:
 * - Trong dev (Vite proxy): kết nối về same-origin (cùng host:port với frontend)
 *   để Vite forward /socket.io → backend:5002. Mobile dùng IP LAN của Vite cũng được.
 * - Khi apiUrl chỉ đến backend trực tiếp (không qua proxy): dùng apiUrl.
 */
const makeSocketUrl = (apiUrl) => {
  // Nếu apiUrl là localhost:5002 hoặc trống, dùng same-origin (qua Vite proxy)
  const base = apiBase(apiUrl);
  if (!base || /localhost/.test(base) || /127\.0\.0\.1/.test(base)) {
...
const CLAUSE_OPTIONS = [...new Set(GAP_CHECKLIST.map((c) => c.clause))]
  .sort()
  .map((cl) => [cl, `§${cl}`]);
...
export default function StepEvidence({ survey, surveyId, apiUrl, setToast, ensureSurveyId }) {
  ...
  const fetchList = useCallback(async () => {
    ...
    p.set("limit", "1000");
    const url = `${ab}/api/surveys/${sid}/evidence?${p}`;
    try {
      let res;
      // Thử lại tối đa 3 lần khi MongoDB trả về 503 (đang khởi động)
      ...
      if (res.status === 503) {
        toast("error", "MongoDB chưa kết nối. Vui lòng thử lại sau vài giây.");
        return;
      }
  ...
  const handleFileUpload = useCallback(
    async (e) => {
      ...
      for (let i = 0; i < files.length; i += 1) {
        ...
        form.append("originalName", files[i].name); // tên UTF-8 gốc, tránh lỗi encode của multer
  ...
  }, [editItem, editForm, sid, ab, toast, fetchList]);
        ...
        {editItem && (
          <div style={{ display: "flex", flexDirection: "column", gap: SPACE.md }}>
            ...
            <div style={{ display: "flex", gap: SPACE.md }}>
              <Btn v="primary" sz="md" onClick={saveEdit}>
                Lưu
              </Btn>
              <Btn v="ghost" sz="md" onClick={() => setEditItem(null)}>
                Hủy
              </Btn>
            </div>
          </div>
