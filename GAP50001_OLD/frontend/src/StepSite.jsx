...
export default function StepSite({ survey, setSurvey }) {
  ...
  zones.forEach(z => {
    ...
    const str = z.consumption.toLowerCase();
    const regex = /([\d,.]+)\s*(kwh|tấn|tấn gas|lit|lít|m3|m³)/g;
    let match;
    while ((match = regex.exec(str)) !== null) {
      ...
      let unit = match[2];
      if (unit.startsWith('tấn')) energyTotals.tan += val;
      else if (unit === 'kwh') energyTotals.kWh += val;
      else if (unit.startsWith('lit') || unit.startsWith('lít')) energyTotals.lit += val;
      else if (unit === 'm3' || unit === 'm³') energyTotals.m3 += val;
    }
  });

  // Calculate total solar excluding "máy nén khí"
  let totalSolar = 0;
  (survey.solar_assessments || []).forEach(s => {
    if (!s.generation) return;
    const segments = s.generation.toLowerCase().split(/(?:\+|;|và|\n)/);
    segments.forEach(seg => {
      if (seg.includes('máy nén khí') || seg.includes('máy nén')) return;
      const regex = /([\d,.]+)\s*kwh/g;
      let match;
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      ...
      <div style={{ background: C.bg2, borderRadius: 10, padding: 16, border: `1px solid ${C.bd1}` }}>
        ...
        <Grid cols={5} gap={10}>
          ...
          <div style={{ background: C.bg3, padding: 12, borderRadius: 8, borderLeft: `4px solid ${C.tealL}` }}>
            <div style={{ fontSize: 12, color: C.t2 }}>Tổng Dầu / Hóa Chất</div>
            <div style={{ fontSize: 20, color: C.t0, fontWeight: 700 }}>
              {new Intl.NumberFormat('en-US').format(energyTotals.lit)} <span style={{fontSize:14, color:C.t3}}>lít</span>
            </div>
          </div>
          <div style={{ background: C.bg3, padding: 12, borderRadius: 8, borderLeft: `4px solid ${C.blueL}` }}>
            <div style={{ fontSize: 12, color: C.t2 }}>Tổng Khí tự nhiên/Nước</div>
            <div style={{ fontSize: 20, color: C.t0, fontWeight: 700 }}>
              {new Intl.NumberFormat('en-US').format(energyTotals.m3)} <span style={{fontSize:14, color:C.t3}}>m³</span>
            </div>
          </div>
