...
export default function StepActions({ survey, setSurvey, checklist: checklistProp }) {
  ...
  const calcPhase = (val) => {
    if (!val) return null;
    if (val.toLowerCase().includes("ngày") || (!isNaN(parseInt(val)) && val.length < 5)) {
    ...
    if (!isNaN(d)) {
      ...
      if (d < today) return "P1"; // Overdue -> P1
  ...
  const openModalAdd = useCallback(item => {
    ...
    const defaultDeadline = sc === 1 ? "30 ngày" : sc === 2 ? "90 ngày" : "180 ngày";
    setModalItem(item);
    setModalAction(null);
    setForm({
      note: r.note || "",
      recommendation: r.recommendation || "",
      responseDeadline: r.deadline || defaultDeadline,
      code: `AP-${item.id}`,
        ...
        {modalItem && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{ padding: 14, background: C.bg3, borderRadius: 10, border: `1px solid ${C.bd0}` }}>
              ...
              <Field label="Ghi chú đánh giá (evidence, phát hiện)">
                <TA value={form.note} onChange={v => setFormField("note", v)} placeholder="Ghi chú khi khảo sát..." rows={2}/>
              </Field>
              <Field label="Đề xuất khắc phục / Khuyến nghị" sx={{ marginTop: 10 }}>
              ...
              <Field label="Hạn hoàn thành đề xuất (từ GAP)" sx={{ marginTop: 10 }}>
            ...
            <div style={{ padding: 14, background: C.bg3, borderRadius: 10, border: `1px solid ${C.teal}30` }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <Field label="Giai đoạn">
                  <Sel value={form.phase} onChange={v => setFormField("phase", v)} options={ACTION_PHASES.map(p => [p.id, p.label.split("—")[0].trim()])}/>
                </Field>
              </div>
