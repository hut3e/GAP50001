...
function clauseScore(clauseNum, responses) {
  const items = GAP_CHECKLIST.filter(i => i.clause.startsWith(clauseNum.replace("§","")));
...
function gapPercent(responses) {
  const total  = GAP_CHECKLIST.length;
  const gaps   = GAP_CHECKLIST.filter(i => (responses[i.id]?.score||0) <= 2).length;
...
function buildCover(d) {
  ...
  const gp = gapPercent(d.responses||{});
...
function buildExecutiveSummary(d) {
  ...
  const critCount = GAP_CHECKLIST.filter(i=>(resp[i.id]?.score||0)===1).length;
  const majCount  = GAP_CHECKLIST.filter(i=>(resp[i.id]?.score||0)===2).length;
  const minCount  = GAP_CHECKLIST.filter(i=>(resp[i.id]?.score||0)===3).length;
  const goodCount = GAP_CHECKLIST.filter(i=>(resp[i.id]?.score||0)>=4).length;
