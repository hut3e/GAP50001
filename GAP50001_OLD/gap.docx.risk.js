...
function buildRiskSection(d, secNum) {
  ...
  RISK_CATEGORIES.forEach((cat, ci) => {
    ...
    cat.items.forEach((risk, ri) => {
      const rd  = riskData[risk.id] || {};
      const l   = rd.likelihood || 0;
      const imp = rd.impact     || 0;
      ...
      catRows.push(new TableRow({ children:[
        ...
        new TableCell({ width:{size:1600}, borders:cb(), shading:sh(C.white), margins:CMs, children:[P(rd.control||"Chưa xác định",{sz:16,col:C.grey1})]}),
        new TableCell({ width:{size:900},  borders:cb(), shading:sh(C.white), margins:CMs, children:[P(rd.recommendation||"—",{sz:15,it:true,col:C.blue})]}),
      ]}));
    });
  ...
  // Top risks summary
  const allRisks = RISK_CATEGORIES.flatMap(cat => cat.items.map(r => ({
    ...r, ...riskData[r.id], catName:cat.name,
    score:(riskData[r.id]?.likelihood||0)*(riskData[r.id]?.impact||0),
