...
const { C, cb, sh, CMs, P, H2, H3, SP } = require("./gap.docx.helpers");
  ...
  try {
    ...
    return [
      ...
      new Paragraph({
        ...
        children: [
          new TextRun({ text: caption, font: "Arial", size: 18, color: C.grey1 }),
        ],
      }),
    ];
  } catch (err) {
    return [P("[Không đọc được ảnh: " + (ev.filename || "") + "]", { sz: 15, col: C.red })];
  }
}
...
function buildEvidenceSection(d, evidenceList, startSec) {
  ...
  items.push(H2(`${startSec}. BẰNG CHỨNG THEO ĐIỀU KHOẢN ĐÁNH GIÁ`));
  items.push(P("Các tài liệu và ảnh bằng chứng thu thập tại hiện trường (chọn file / Camera Laptop / đồng bộ điện thoại qua Socket.IO), gắn với từng điều khoản. Ảnh bố trí theo bảng 2 cột.", { sz: 18, col: C.grey1 }));
  items.push(SP());
        ...
        for (let i = 0; i < COLS_IMAGE_TABLE; i++) {
          ...
          if (!ev) {
            cells.push(new TableCell({
              ...
              shading: sh(C.ash),
              margins: CMs,
              children: [P("—", { sz: 16, col: C.grey3, c: true })],
            }));
            continue;
          }
          ...
          const cellContent = fs.existsSync(fullPath)
            ...
            : [P("[File không tồn tại: " + (ev.filename || "") + "]", { sz: 15, col: C.red })];
          cells.push(new TableCell({
            width: { size: colWidth, type: WidthType.DXA },
            borders: cb(),
            shading: sh(C.white),
    ...
    if (docList.length > 0) {
      const docRows = [
        new TableRow({
          tableHeader: true,
          children: [
            new TableCell({ width: { size: 4680, type: WidthType.DXA }, borders: cb(), shading: sh(C.blue), margins: CMs, children: [P("Tài liệu đính kèm", { sz: 16, bold: true, col: C.white, c: true })] }),
            new TableCell({ width: { size: 4680, type: WidthType.DXA }, borders: cb(), shading: sh(C.blue), margins: CMs, children: [P("Ghi chú", { sz: 16, bold: true, col: C.white, c: true })] }),
          ],
        }),
      ];
      for (const ev of docList) {
        docRows.push(new TableRow({
          children: [
            new TableCell({ width: { size: 4680, type: WidthType.DXA }, borders: cb(), shading: sh(C.ash), margins: CMs, children: [P("📄 " + (ev.originalName || ev.filename || "—"), { sz: 16 })] }),
            new TableCell({ width: { size: 4680, type: WidthType.DXA }, borders: cb(), shading: sh(C.white), margins: CMs, children: [P(ev.note || "—", { sz: 16 })] }),
          ],
        }));
      }
      items.push(new Table({
