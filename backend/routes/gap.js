...
router.post("/generate", async (req, res) => {
  try {
    ...
    // Dùng checklist từ DB (nếu có), fallback về static default
    let checklist = DEFAULT_CHECKLIST;
    try {
      const dbItems = await ChecklistItem.find().sort({ order: 1, id: 1 }).lean();
      if (dbItems.length > 0) checklist = dbItems;
    } catch (_) {}
    const buf = await generateGapReport(d, evidenceList, checklist);
