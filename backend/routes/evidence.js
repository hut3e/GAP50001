...
// PUT /api/surveys/:surveyId/evidence/:id — update metadata (clauseId, note, context) and optionally replace file
router.put("/:id", requireMongo, (req, res, next) => {
  upload.single("file")(req, res, (multerErr) => {
    if (multerErr) {
      const msg = multerErr.code === "LIMIT_FILE_SIZE" ? "File vượt quá 25MB." : multerErr.message || "Lỗi upload file.";
      return res.status(400).json({ error: msg });
    }
    next();
  });
  ...
  try {
    ...
    if (req.file) {
      const isImage = (req.file.mimetype || "").startsWith("image/");
      update.type = isImage ? "image" : "document";
      update.filename = req.file.filename;
      update.originalName = req.file.originalname || req.file.filename;
      update.mimeType = req.file.mimetype || "";
      update.size = req.file.size || 0;
      update.path = path.join(String(surveyId), req.file.filename);
      update.source = "upload";
    }

    const doc = await Evidence.findOne({ _id: id, surveyId });
    if (!doc) return res.status(404).json({ error: "Evidence not found" });

    // If new file uploaded, delete the old file safely
    if (req.file && doc.path) {
      const oldPath = path.join(UPLOAD_ROOT, doc.path);
      if (fs.existsSync(oldPath)) {
        try { fs.unlinkSync(oldPath); } catch (e) { console.warn("[evidence] unlink old file failed", e.message); }
      }
    }
