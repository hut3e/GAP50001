...
import { Btn, Tag, Field, Input, TA, Sel, Modal } from "./gap.atoms.jsx";
...
export default function LiteGapAudit({ open, onClose, survey, setSurvey, apiUrl, onSave, setToast }) {
  ...
  return (
    <div style={{
      ...
      {/* Tab bar */}
      <div style={{
        display: "flex", gap: 0, background: C.bg2, borderBottom: `1px solid ${C.bd0}`,
      }}>
        {TABS.map(t => (
      ...
      <div style={{ flex: 1, overflow: "auto", padding: 20, background: C.bg0 }}>
          ...
          <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{ background: C.bg2, borderRadius: 12, padding: 20, border: `1px solid ${C.bd0}` }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <InfoRow label="Tên tổ chức" value={client.name} accent />
                <InfoRow label="Cơ sở / Nhà máy" value={client.site} />
                <InfoRow label="Địa chỉ" value={client.address} full />
                <InfoRow label="Ngành nghề" value={client.industry} />
                <InfoRow label="NL tiêu thụ" value={client.annual_energy ? `${client.annual_energy} TOE/năm` : ""} />
                <InfoRow label="Người đại diện" value={client.representative_name} />
                <InfoRow label="Chức vụ đại diện" value={client.representative_position} />
                {(client.contact_persons || []).map((c, i) => (
                  <InfoRow key={i} label={`Liên hệ ${i+1}`} value={`${c.full_name || ""} — ${c.position || ""} — ${c.phone || ""}`} full />
                ))}
              </div>
            </div>
            ...
            <div style={{ background: C.bg2, borderRadius: 12, padding: 20, border: `1px solid ${C.bd0}` }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <InfoRow label="Tổ chức tư vấn" value={verifier.org} accent />
                <InfoRow label="Trưởng đoàn" value={verifier.lead} />
                <InfoRow label="Thành viên" value={verifier.team} full />
                <InfoRow label="Số chứng chỉ" value={verifier.cert_no} />
                <InfoRow label="Tiêu chuẩn áp dụng" value={verifier.std_applied} full />
              </div>
            </div>
            ...
            <div style={{ background: C.bg2, borderRadius: 12, padding: 20, border: `1px solid ${C.bd0}` }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                <InfoRow label="Mã khảo sát" value={meta.ref_no} accent />
                <InfoRow label="Ngày khảo sát" value={meta.survey_date} />
                <InfoRow label="Phiên bản" value={meta.version || "v1.0"} />
              </div>
            </div>
          </div>
        )}
        ...
        {tab === "check" && (
          <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            ...
            {Object.entries(clauseGroups).sort(([a],[b]) => parseFloat(a) - parseFloat(b)).map(([clause, items]) => {
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, background: C.bg2 }}>
                      <thead>
                        <tr style={{ background: C.bg3 }}>
                          <th style={{ padding: "6px 10px", textAlign: "left", borderBottom: `1px solid ${C.bd0}`, width: 60 }}>ID</th>
                          <th style={{ padding: "6px 10px", textAlign: "left", borderBottom: `1px solid ${C.bd0}` }}>Yêu cầu</th>
                        ...
                        {items.map((item, idx) => {
                          ...
                          return (
                            <tr key={item.id} style={{ background: idx % 2 ? C.bg2 : "transparent" }}>
                              <td style={{ padding: "6px 10px", borderBottom: `1px solid ${C.bd2}`, fontFamily: "'Fira Code',monospace", color: C.blueL, fontSize: 12 }}>{item.id}</td>
      ...
      <Modal open={siteModal.open} onClose={() => setSiteModal({ open: false, index: null })}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <Field label="Hiện trạng vận hành">
            <TA value={siteForm.status} onChange={v => setSiteForm(f => ({ ...f, status: v }))} rows={2} placeholder="Mô tả hiện trạng máy móc, thiết bị, vận hành..." />
          </Field>
          <Field label="Rủi ro phát hiện">
            <TA value={siteForm.risk} onChange={v => setSiteForm(f => ({ ...f, risk: v }))} rows={2} placeholder="Rò rỉ hơi, rung không đồng tâm, nhiệt độ cao..." />
          </Field>
          <Field label="Cơ hội cải tiến năng lượng">
            <TA value={siteForm.opportunity} onChange={v => setSiteForm(f => ({ ...f, opportunity: v }))} rows={2} placeholder="Lắp biến tần, bảo ôn đường ống, thay đèn LED..." />
          </Field>
...
/** Row hiển thị thông tin read-only */
function InfoRow({ label, value, full, accent }) {
  return (
    <div style={{ gridColumn: full ? "1/-1" : undefined, padding: "6px 0" }}>
      <div style={{ fontSize: 11, color: C.t3, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 2 }}>{label}</div>
      <div style={{ fontSize: 14, color: accent ? C.blueL : C.t0, fontWeight: accent ? 700 : 400 }}>{value || "—"}</div>
    </div>
  );
}
