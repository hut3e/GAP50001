        ...
        return (
          <div key={pr.id} style={{ background:C.bg2, borderRadius:9,
            ...
            <div style={{ display:"flex", alignItems:"flex-start", gap:10, marginBottom:8 }}>
              <span style={{ fontFamily:"'Fira Code',monospace", fontSize:12.5, color:C.blueL, flexShrink:0, marginTop:2 }}>{pr.id}</span>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:16, fontWeight:700, color:C.t0 }}>{pr.name}</div>
            ...
            <div style={{ marginBottom:sc>0?8:0 }}>
              <div style={{ fontSize:11.5, color:C.t2, fontWeight:700, marginBottom:4, textTransform:"uppercase", letterSpacing:.5 }}>Điểm đánh giá quá trình:</div>
            ...
            {sc > 0 && (
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginTop:8 }}>
                {[
                  ...
                  <div key={field}>
                    <div style={{ fontSize:11.5, color:C.t2, fontWeight:700, marginBottom:3, textTransform:"uppercase" }}>{label}</div>
                    <textarea value={pd[field]||""} onChange={e=>setProc(pr.id,field,e.target.value)}
                      ...
                      style={{ width:"100%", background:bg, border:`1px solid ${bc}30`,
                        borderRadius:5, padding:"5px 8px", color:C.t0, fontSize:13.5,
